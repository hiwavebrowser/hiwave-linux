...
pub enum WebViewEngine {
    /// WebKit (Linux GTK)
    WebKit,
    /// RustKit (pure Rust browser engine)
    RustKit,
}
...
pub struct PlatformCapabilities {
    /// Whether a native menu is required for clipboard operations
...
pub trait PlatformManager: Send + Sync {
    ...
    ///
    /// This adds the menu bar to the window.
    ...
    /// Show a file in the system's file manager (Nautilus/Dolphin/etc)
    ///
    ...
    ///
    /// Platform-specific heuristics for popup filtering.
...
/// Get the platform manager for Linux
